...
public class CircleBarsTexture extends IBarsRenderer {
    ...
    public CircleBarsTexture(String vs, String fs, String texPath, float radius) {
        super(vs, fs);
        mRadius = radius;
    ...
    private void drawBars() {
        ...
        for (int i = 0; i < mBarCount; i++) {
            ...
            mModelMatrix.translate(cos*radius, (float) (sin*radius ), 0);
            float extraAngel = (float) Math.atan(mBarWidth/2/radius);
            mModelMatrix.rotate(angelRadius + extraAngel, 0, 0, 1);
            mModelMatrix.scale(mScale.x * th/1.5f, mScale.y*1.8f, 1);
